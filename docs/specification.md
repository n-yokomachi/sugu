...
| 演算子 | 意味 | 例 |
|---|---|---|
| `++` | インクリメント（後置） | `x++` → 更新前の値を返し、`x` を `+1` |
| `--` | デクリメント（後置） | `x--` → 更新前の値を返し、`x` を `-1` |
...
> 注: 後置演算子のみサポートしています（前置 `++x` は未対応）。対象は変数のみで、数値型以外に使用するとエラーになります。`const` 変数への適用もエラーです。

### 複合代入演算子

| 演算子 | 意味 | 例 |
|---|---|---|
| `+=` | 加算代入 | `x += 5` は `x = x + 5` と同等 |
| `-=` | 減算代入 | `x -= 3` は `x = x - 3` と同等 |
| `*=` | 乗算代入 | `x *= 2` は `x = x * 2` と同等 |
| `/=` | 除算代入 | `x /= 4` は `x = x / 4` と同等 |
| `%=` | 剰余代入 | `x %= 3` は `x = x % 3` と同等 |
...
// インデックス式にも使用可能
mut arr = [10, 20, 30];
arr[0] += 5;     // arr = [15, 20, 30]
```

> 注: `const` 変数への複合代入はエラーです。`/=` および `%=` で右辺が 0 の場合はゼロ除算エラーになります。
...
配列・文字列ともに負のインデックスで末尾からアクセスできます：

```javascript
const arr = [10, 20, 30];
outln(arr[-1]);  // 30（最後の要素）
outln(arr[-2]);  // 20（最後から2番目）
outln(arr[-3]);  // 10（最初の要素）
outln(arr[-4]);  // null（範囲外）
...
配列・文字列の部分範囲を取得できます（Python スタイル）：

```javascript
const arr = [1, 2, 3, 4, 5];

outln(arr[1:3]);   // [2, 3]     （開始:終了）
outln(arr[:3]);    // [1, 2, 3]  （先頭から）
outln(arr[2:]);    // [3, 4, 5]  （末尾まで）
outln(arr[:]);     // [1, 2, 3, 4, 5]（全体コピー）
outln(arr[-2:]);   // [4, 5]     （負のインデックス）
...
> 注: スライスは常に新しい配列/文字列を返します（元のデータは変更されません）。範囲外のインデックスは自動的にクランプされ、エラーにはなりません。
...
arr[-1] = 99;     // arr は [10, 2, 99]（負のインデックスも使用可能）
```
...
| `contains(arr, x)` | 要素が含まれるか判定 | `contains([1,2,3], 2)` → `true` |
| `concat(arr1, arr2, ...)` | 複数の配列を結合した新配列を返す | `concat([1,2], [3,4])` → `[1,2,3,4]` |

> 注: `contains` は文字列にも使用でき、部分文字列の包含を判定します（`contains("hello", "ell")` → `true`）。`concat` は元の配列を変更しません。
