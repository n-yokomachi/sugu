...
- [x] 配列リテラル `[1, 2, 3]` が動作する
- [x] マップリテラル `{"a": 1}` が動作する
- [x] インデックスアクセス `arr[0]`, `map["key"]` が動作する
- [x] エラーメッセージに行番号が含まれる
- [x] `len()`, `push()` などの組み込み関数が動作する
- [x] 全テストが成功する
